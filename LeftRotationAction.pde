...
class LeftRotationAction extends Action{
  /*
  ...
  */
  LeftRotationAction(boolean fancy, int center, float amount, LRN lrn, float speed) {
    isFancy = fancy;
    rotationVert = center;
    rotationSpeed = speed;
  ...
  */
  ...
  boolean perform() {
    ...
    if (shouldContinue) {
      ...
      framework.rotateLeftSubFrame(rotationVert, rotateBy);
      background(255);
